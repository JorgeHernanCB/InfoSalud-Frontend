...
export class ProveedoresComponent implements OnInit {
  ...
  public proveedorsForm = new FormGroup({
    id: new FormControl<number>(0, { nonNullable: true }), // Asegúrate de que el ID sea un número requerido y no pueda ser null
    name: new FormControl<string>('', { nonNullable: true, validators: Validators.required }),
    codigo: new FormControl<string>(''),
    typePerson: new FormControl<string>('', {nonNullable: true, validators: Validators.required }),
    typeDocument: new FormControl<string>('', {nonNullable: true, validators: Validators.required }),
    numberDocument: new FormControl<string>('', {nonNullable: true, validators: Validators.required }),
    city: new FormControl<string>('', {nonNullable: true, validators: Validators.required }),
    departament: new FormControl<string>('', {nonNullable: true, validators: Validators.required }),
  ...
  editProveedor(proveedor: proveedores){
    this.proveedor = {...proveedor};
    this.proveedorsForm.patchValue({
      // id: proveedor.id,
      name: proveedor.name,
      // codigo: proveedor.codigo,
      typePerson: proveedor.typePerson,
      typeDocument: proveedor.typeDocument,
      numberDocument: proveedor.numberDocument,
      city: proveedor.city,
      departament: proveedor.departament,
      // date_start: proveedor.date_start,
      // date_finish: proveedor.date_finish
    });
    this.proveedorDialog = true;
  }
  ...
  saveProveedor() {
    ...
    if (this.proveedorsForm.valid) {
      ...
      if (this.proveedor.id) {
        ...
        const index = this.proveedores.findIndex(p => p.id === this.proveedor.id);
        if (index !== -1) {
        ...
        proveedorActualizado.id = this.createId(); // Asignar un nuevo ID
      ...
      this.proveedorsForm.reset();
      this.proveedor = {} as proveedores; // Limpiar objeto de proveedor
    }
  }
